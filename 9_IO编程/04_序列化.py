...
b'\x80\x04\x95$\x00\x00\x00\x00\x00\x00\x00}\x94(\x8c\x04name\x94\x8c\x03Bob\x94\x8c\x03age\x94K\x14\x8c\x05score\x94K<u.'
"""
...
# 目录下多了一个dump.txt文件，看看写入的dump.txt文件，一堆乱七八糟的内容，这些都是Python保存的对象内部信息。
...
"""
>>> import json
>>> d = dict(name='Albert', age=16, score=99)
>>> json.dumps(d)
'{"name": "Albert", "age": 16, "score": 99}'
"""
...
Traceback (most recent call last):
  ...
TypeError: <__main__.Student object at 0x......> is not JSON serializable
"""

# 错误的原因是Student对象不是一个可以序列化为JSON的对象。
#
...
# 不过，下次如果遇到一个Teacher类的实例，照样无法序列化为JSON。可以偷个懒，把任意class的实例变为dict：
"""
...
#
# 同理，如果要把JSON反序列化为一个Student对象实例，loads()方法首先转换出一个dict对象，然后，我们传入的object_hook函数负责把dict转换为Student实例：
"""
def dict2student(d):
