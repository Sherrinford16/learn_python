...
"""
# 生成器都是Iterator对象，但list、dict、str虽然是Iterable，却不是Iteration。
# 把list、str、dict等Iterable变成Iteration可以使用iter()函数：
"""
...
"""
# 提出问题：为什么list、dict、str等数据类型不是Iterator？
# 因为python的Iterator对象表示的是一个数据流，Iterator对象可以被next()函数调用并不断返回下一个数据，
# 直到没有数据时抛出StopIteration错误。可以把这个数据流看作是一个有序序列，但我们却不能提前知道序列的长度，
# 只能不断通过next()函数实现按需计算下一个数据，所以Iterator的计算时惰性的，
# 只有在需要返回下一个数据时它才会计算
# 
...
for m in [1, 2, 3, 4, 5]:
    pass
# 实际上完全等价于：
it = iter([1, 2, 3, 4, 5])  # 首先获得一个Iterator对象
# 循环：
while True:
    try:
        # 获得下一个值：
